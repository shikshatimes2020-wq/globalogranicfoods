...
app.post('/api/orders', async (req, res) => {
  try {
    ...
    // ── ফ্রি ডেলিভারি সার্ভার-সাইড চেক ──
    const [fdSetting, fdMinSetting] = await Promise.all([
      SiteSettings.findOne({ key: 'freeDelivery' }),
      SiteSettings.findOne({ key: 'freeDeliveryMinOrder' }),
    ]);
    const fdVal = fdSetting?.value;
    const minOrder = parseInt(fdMinSetting?.value) || 0;
    const subTotal = (parseFloat(unitPrice) || 0) * (parseInt(quantity) || 1);
    let resolvedDelivery = parseFloat(deliveryCharge) || 0;
    if (fdVal === true || fdVal === 'true') {
      resolvedDelivery = 0; // সব অর্ডারে ফ্রি
    } else if (fdVal === 'conditional' && subTotal >= minOrder) {
      resolvedDelivery = 0; // শর্তসাপেক্ষ ফ্রি
    }
    const resolvedTotal = subTotal + resolvedDelivery;
    ...
    while (attempts < 5) {
      try {
        ...
        order = await Order.create({ orderNumber, productId, productName, weight, quantity, unitPrice, deliveryCharge: resolvedDelivery, totalPrice: resolvedTotal, customerName, phone, address, deliveryArea, notes });
        break; // success
      } catch (dupErr) {
